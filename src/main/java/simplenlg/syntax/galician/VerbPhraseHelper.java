...
class VerbPhraseHelper extends simplenlg.syntax.VerbPhraseHelper {
     ...
     */
    protected void realiseComplements(SyntaxProcessor parent, PhraseElement phrase, ListElement realisedElement) {
...
//                    .equals(interrType));
            ...
            if (!vgComponents.empty() || frontVG != null && isCopular(frontVG)) {
                ...
                vgComponents.push(new InflectedWordElement(
                        "non", LexicalCategory.ADVERB)); //$NON-NLS-1$
            } else {
        ...
        if (phrase.getFeatureAsBoolean(LexicalFeature.REFLEXIVE)) {
            ...
            switch (p) {
                case FIRST:
                    switch (n) {
                        case PLURAL:
                            pronoun = new InflectedWordElement("nós", LexicalCategory.PRONOUN);
                            break;
                        default:
                            pronoun = new InflectedWordElement("eu", LexicalCategory.PRONOUN);
                    }
                    break;
                case SECOND:
                    switch (n) {
                        case PLURAL:
                            pronoun = new InflectedWordElement("vós", LexicalCategory.PRONOUN);
                            break;
                        default:
                            pronoun = new InflectedWordElement("ti", LexicalCategory.PRONOUN);
                    }
                    break;
                default:
                    switch (n) {
                        case PLURAL:
                            pronoun = new InflectedWordElement("eles", LexicalCategory.PRONOUN);
                            break;
                        default:
                            pronoun = new InflectedWordElement("el", LexicalCategory.PRONOUN);
                    }
            }
            vgComponents.push(frontVG);
