...
public class AdjectivePhraseTest extends SimpleNLG4Test {
    ...
    public void testAdj() {
        ...
        Assert.assertEquals("increíblemente salaz e sorprendentemente bonito", //$NON-NLS-1$
        ...
        Assert.assertEquals("increíblemente salaz ou sorprendentemente bonito", //$NON-NLS-1$
                this.realiser.realise(coordap).getRealisation());
        ...
        CoordinatedPhraseElement coord2 = this.phraseFactory.createCoordinatedPhrase(coordap,
                this.stunning);
        Assert.assertEquals(
                "increíblemente salaz ou sorprendentemente bonito e estupendo", //$NON-NLS-1$
        ...
        Assert.assertEquals(
                "seriamente e innegablemente increíblemente salaz ou sorprendentemente bonito e estupendo", //$NON-NLS-1$
                this.realiser.realise(coord2).getRealisation());
        ...
        Assert.assertEquals(
                "increíblemente salaz, sorprendentemente bonito ou estupendo", //$NON-NLS-1$
                this.realiser.realise(coordap).getRealisation());
    ...
    public void testAdv() {
        ...
        adv.addPreModifier("moi"); //$NON-NLS-1$

        Assert.assertEquals("John come moi rápido", this.realiser.realise( //$NON-NLS-1$
                sent).getRealisation());
    ...
    public void testMultipleModifiers() {
        PhraseElement np = this.phraseFactory
                .createNounPhrase(this.lexicon.getWord("mensaxe",
                        LexicalCategory.NOUN));
        np.addPreModifier(this.lexicon.getWord("activo",
                LexicalCategory.ADJECTIVE));
        np.addPreModifier(this.lexicon.getWord("temperatura",
                LexicalCategory.ADJECTIVE));
        Assert.assertEquals("activa, temperatura mensaxe", this.realiser.realise(np).getRealisation());
