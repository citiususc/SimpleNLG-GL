...
public abstract class SimpleNLG4Test extends TestCase {
    ...
    protected void setUp() {
        lexicon = new simplenlg.lexicon.galician.XMLLexicon();  // built in lexicon

        this.phraseFactory = new NLGFactory(this.lexicon);
        this.realiser = new simplenlg.realiser.galician.Realiser(this.lexicon);

        this.man = this.phraseFactory.createNounPhrase("o", "home"); //$NON-NLS-1$ //$NON-NLS-2$
        this.woman = this.phraseFactory.createNounPhrase("o", "muller");  //$NON-NLS-1$//$NON-NLS-2$
        this.dog = this.phraseFactory.createNounPhrase("o", "can"); //$NON-NLS-1$ //$NON-NLS-2$
        this.boy = this.phraseFactory.createNounPhrase("o", "neno"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        this.np4 = this.phraseFactory.createNounPhrase("o", "roca"); //$NON-NLS-1$ //$NON-NLS-2$
        this.onTheRock.addComplement(this.np4);

        this.behindTheCurtain = this.phraseFactory.createPrepositionPhrase("tras"); //$NON-NLS-1$
        this.np5 = this.phraseFactory.createNounPhrase("o", "cortina"); //$NON-NLS-1$ //$NON-NLS-2$
        this.behindTheCurtain.addComplement(this.np5);

        this.inTheRoom = this.phraseFactory.createPrepositionPhrase("en"); //$NON-NLS-1$
        this.np6 = this.phraseFactory.createNounPhrase("o", "habitación"); //$NON-NLS-1$ //$NON-NLS-2$
        this.inTheRoom.addComplement(this.np6);

        this.underTheTable = this.phraseFactory.createPrepositionPhrase("baixo"); //$NON-NLS-1$
        this.underTheTable.addComplement(this.phraseFactory.createNounPhrase("o", "mesa")); //$NON-NLS-1$ //$NON-NLS-2$

        this.proTest1 = this.phraseFactory.createNounPhrase("o", "cantante"); //$NON-NLS-1$ //$NON-NLS-2$
        this.proTest2 = this.phraseFactory.createNounPhrase("algunha", "persoa"); //$NON-NLS-1$ //$NON-NLS-2$

        this.kick = this.phraseFactory.createVerbPhrase("golpear"); //$NON-NLS-1$
        this.kiss = this.phraseFactory.createVerbPhrase("bicar"); //$NON-NLS-1$
        this.walk = this.phraseFactory.createVerbPhrase("camiñar"); //$NON-NLS-1$
        this.talk = this.phraseFactory.createVerbPhrase("falar"); //$NON-NLS-1$
        ...
        this.say = this.phraseFactory.createVerbPhrase("dicir"); //$NON-NLS-1$
    }
