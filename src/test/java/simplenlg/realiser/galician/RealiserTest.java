 ...
 */
package simplenlg.realiser.galician;
...
public class RealiserTest {
    ...
    private simplenlg.realiser.galician.Realiser realiser = null;
    ...
    public void multipleNLGElementListRealiserTest() {
        ...
        NPPhraseSpec subject_2 = nlgFactory.createNounPhrase("o", "can");
        ...
        Assert.assertEquals("O gato saltando no mostrador.", realisedElements.get(0).getRealisation());
        Assert.assertEquals("O can correndo no mostrador.", realisedElements.get(1).getRealisation());
    ...
    public void correctPluralizationWithPossessives() {
        NPPhraseSpec sisterNP = nlgFactory.createNounPhrase("a", "irmá");
...
//        possNP.setFeature(Feature.POSSESSIVE, true);
        ...
        Assert.assertEquals("a irmá de Albert Einstein",
                realiser.realise(sisterNP).getRealisation());
        sisterNP.setPlural(true);
        Assert.assertEquals("as irmás de Albert Einstein",
                realiser.realise(sisterNP).getRealisation());
        ...
        Assert.assertEquals("a irmá de el",
                realiser.realise(sisterNP).getRealisation());
        sisterNP.setPlural(true);
        Assert.assertEquals("as irmás de el",
                realiser.realise(sisterNP).getRealisation());
    }
